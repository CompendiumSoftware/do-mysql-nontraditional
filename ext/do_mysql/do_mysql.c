...
// For really anscient MySQL versions we don't attempt any strictness
  ...
  if (mysql_get_server_version(db) < 50000) {
    ...
    do_mysql_cCommand_execute(Qnil, self, db, rb_str_new2("SET SESSION sql_mode = 'ANSI,NO_BACKSLASH_ESCAPES,NO_DIR_IN_CREATE,NO_ENGINE_SUBSTITUTION,NO_UNSIGNED_SUBTRACTION'"));
  }
#endif
